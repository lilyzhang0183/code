...
if __name__ == '__main__':
    trainset = open('train.txt', encoding='utf-8')     #读取训练集
    testset = open('test.txt', encoding='utf-8')       #读取测试集
    # trainlist = []
    ...
    test_lines = testset.readlines()[:10]
    seg_lines = []
    for line in test_lines:
        ...
        for i in range(len(seg)):
            lists = lists + seg[i] + ' '
        seg_lines.append(lists)

    for line, seg_line in zip(test_lines, seg_lines):
        print(line, seg_line)
